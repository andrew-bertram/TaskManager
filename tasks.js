...
// PERSONAL TASK MANAGER (Done on my own)
...
// FROM CLASS (Completed version done with class)
...
// For storingthe number of the task to be removed
let num;

// For storing the value of the removed task
let removed;

// Displays the menu for the user to select an option. Also, sets the user's response to the userInput variable. Also, converting to UPPERCASE.
...
while (userInput !== "CLOSE") {
    ...
    if (userInput === "TASKS") {
        ...
        while (true) {
            ...
            // Prompts the user to enter a number and stores their response. Also, using \n to create a new line. Finally, subtracting 1 from the user's entry so it matches the index in the tasks array. (NOTE: JavaScript will attempt to convert a strin into a number when you try to subtract from it. Just like ParseInt. So ParseInt is not actually necessary here.)
